...
（JSONやコードブロックや表形式は禁止、プレーンテキストで出力）
...
- **ホテル名／飲食店名／体験施設名**は必ず正式名称を記載する（「ホテル名」「店名」「Example」「○○」などのダミーは禁止）
- **日程表は「↓」で区切り**、ホテルや食事・体験・実用ガイドは**棒線（──────────────────────────────）**で区切る
- **丸数字①②③…**をホテル・食事・体験ごとに振る
- 各URLは「🔗 公式」「📍 Googleマップ」で表記する
- lang=jaなら日本語、lang=enなら英語で一貫出力
...
2️⃣ 日程表（Day1〜帰着まで）
- 各日6ブロック以上  
- その日の最初のブロックのみ「DayN」を表示  
- **各ブロックの最後に必ず「↓」を1行で入れる**  
- 画像URLや📸行は一切出さない  
- 昼食・夕食は店名必須＋価格・営業時間・GoogleマップURLも記載  
- 各日1件、雨天時代替（屋内）も書く  
...
🕰 営業：時間／休：定休  
↓
（次ブロック…同日の中ではDayを繰り返さない）

──────────────────────────────

3️⃣ 実用ガイド
各施設・店舗・体験に①②③…の番号を振り、各ブロックは棒線で区切る。
